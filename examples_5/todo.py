...
class Todo(object):
    ...
    def _index_by_text(self, text):
        """Get index of todo item by its text."""
        return [el[0] for el in self._todos].index(text)
    ...
    def add(self, text):
        ...
        try:
            index = self._index_by_text(text)
            return index
        except ValueError:
            # Item with `text` is not in list
            pass

        new_index = len(self._todos)
        self._todos.append((text, Todo.PENDING))
        return new_index
    ...
    def status_by_index(self, index):
        """Get status of todo task by its index."""
        return self._todos[index][1]

    def status_by_text(self, text):
        """Get status of todo task by its text."""
        index = self._index_by_text(text)
        return self.status_by_index(index)

    def mark_completed_by_index(self, index):
        """Mark item as 'completed' by its index."""
        todo_text = self._todos[index][0]
        self._todos[index] = (todo_text, Todo.COMPLETED)

    def mark_completed_by_text(self, text):
        """Mark item as 'completed' by its text."""
        index = self._index_by_text(text)
        self.mark_completed_by_index(index)

    def next_pending(self):
        return [el[0] for el in self._todos if el[1] == Todo.PENDING][0]
